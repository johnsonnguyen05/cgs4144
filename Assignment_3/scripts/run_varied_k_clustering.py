#!/usr/bin/env python3
"""Run clustering with different k values and save results."""
import os
import sys
import pandas as pd
import numpy as np
...
def main():
    ...
    os.makedirs(varied_k_dir, exist_ok=True)
    
    # Load data
    expr = load_expression_data(input_file)
    ...
    print("\n" + "="*60)
    print("RUNNING CLUSTERING WITH DIFFERENT K VALUES")
    print("="*60)
    ...
    for method in methods:
        ...
        method_dir = os.path.join(varied_k_dir, method)
        os.makedirs(method_dir, exist_ok=True)
        
        for k in k_values:
            print(f"\nRunning {method} with k={k}")
            labels, expr_scaled = run_clustering(expr, method, k)
            
            # Save labels
            out_labels = os.path.join(method_dir, f"k{k}_labels.tsv")
            ...
            cluster_counts = analyze_cluster_counts(labels, f"{method.upper()} k={k}")
            ...
            plt.title(f"{method.upper()} (k={k}) on top-5000 genes")
            plt.legend(title="cluster", loc="best")
            
            out_plot = os.path.join(method_dir, f"k{k}_pca.png")
            plt.tight_layout()
