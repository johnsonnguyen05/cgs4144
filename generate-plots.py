...
# Load metadata
# -------------------------
metadata = pd.read_csv("refinebio/SRP119064/metadata_SRP119064.tsv", sep="\t")  # update filename if needed
metadata = metadata.set_index('refinebio_accession_code')

# -------------------------
# Define groups: Trem2KO vs WT
# -------------------------
groups = metadata['refinebio_subject'].reindex(expr.index)
groups = groups.fillna("Unknown")  # handle missing values
groups = groups.apply(lambda x: 'Trem2KO' if 'Trem2KO' in str(x) else 'WT')
...
sns.scatterplot(x=pca_result[:,0], y=pca_result[:,1], hue=groups, palette="Set1", s=100)
...
plt.figure(figsize=(6,5))
sns.scatterplot(x=tsne_result[:,0], y=tsne_result[:,1], hue=groups, palette="Set1", s=100)
plt.xlabel("t-SNE 1")
plt.ylabel("t-SNE 2")
...
plt.figure(figsize=(6,5))
sns.scatterplot(x=umap_result[:,0], y=umap_result[:,1], hue=groups, palette="Set1", s=100)
plt.xlabel("UMAP 1")
plt.ylabel("UMAP 2")
